...
public class MessageConsole {
	 ...
	 * messages.
	 *
	 * The messages can either be appended to the end of the console or inserted as
	...
	/*
	 * Redirect the output from the standard output to the console using the default
	 * text color and null PrintStream
	 */
	 ...
	 * specified color and PrintStream. When a PrintStream is specified the message
	 * will be added to the Document before it is also written to the PrintStream.
	 */
	...
	/*
	 * Redirect the output from the standard error to the console using the default
	 * text color and null PrintStream
	 */
	...
	/*
	 * To prevent memory from being used up you can control the number of lines to
	 * display in the console
	 *
	 * This number can be dynamically changed, but the console will only be updated
	 * the next time the Document is updated.
	 */
	...
	/*
	 * Class to intercept output from a PrintStream and add it to a Document. The
	 * output can optionally be redirected to a different PrintStream. The text
	...
	class ConsoleOutputStream extends ByteArrayOutputStream {
		...
		/*
		 * Override this method to intercept the output text. Each line of text output
		 * will actually involve invoking this method twice:
		 *
		...
		public void flush() {
			...
			if (logTime && message.length() != 0) {
				message = String.format("[%s]: %s", df.format(new Date()), message);
			}
		...
		/*
		 * We don't want to merge the new message with the existing message so the line
		 * will be inserted as:
		 *
		 * message + newLine
		 */
		...
		/*
		 * The message and the newLine have been added to the buffer in the appropriate
		 * order so we can now update the Document and send the text to the optional
		 * PrintStream.
		 */
