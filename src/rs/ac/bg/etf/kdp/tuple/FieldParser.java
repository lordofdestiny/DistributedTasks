...
class FieldParser {
	public static Optional<? extends Serializable> parse(String field) {
		...
		if ((value = parseString(field)).isPresent())
			return value;
		if ((value = parseCharacter(field)).isPresent())
			return value;
		if ((value = parseClass(field)).isPresent())
			return value;
		if ((value = parseInteger(field)).isPresent())
			return value;
		if ((value = parseDouble(field)).isPresent())
			return value;
		if ((value = parseBoolean(field)).isPresent())
			return value;
