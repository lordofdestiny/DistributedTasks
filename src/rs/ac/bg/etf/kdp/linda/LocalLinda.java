...
public class LocalLinda implements Linda {
	...
	public void eval(final String className, final Object[] ctorParams, final String methodName,
			final Object[] methodsPrams) {
		final var t = new Thread(() -> {
			try {
				final var threadClass = Class.forName(className);
				final var ctorParamTypes = Stream.of(ctorParams).map(Object::getClass)
						.toArray(Class[]::new);
				...
				final var methodParamTypes = Stream.of(methodsPrams).map(Object::getClass)
						.toArray(Class[]::new);

				threadClass.getMethod(methodName, methodParamTypes).invoke(runningThread,
						methodsPrams);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public synchronized void in(String[] tuple) {
		while (true) {
			final var value = tupleSpace.stream().filter(data -> equals(tuple, data)).findFirst();
			if (value.isPresent()) {
	...
	private boolean equals(String[] a, String[] b) {
		...
		return IntStream.range(0, a.length).filter(i -> a[i] != null)
