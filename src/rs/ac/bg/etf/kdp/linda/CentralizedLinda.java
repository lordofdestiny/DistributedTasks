...
public class CentralizedLinda implements Linda {
	...
	public void out(String[] tuple) {
		...
		try {
			...
			readConditions.parallelStream().filter(data::matches).forEach(at -> at.setValue(data));

			takeConditions.parallelStream().filter(data::matches).findAny()
	...
	public boolean inp(String[] tuple) {
		final var template = new Tuple(tuple);
		final var data = tupleSpace.parallelStream().filter(t -> t.matches(template)).findAny();
	...
	private Tuple getOrWaitOn(Tuple template, List<AwaitableTuple> list) {
		return tupleSpace.parallelStream().filter(tup -> tup.matches(template)).findAny()
				.orElseGet(() -> {
	...
	public void eval(String className, Object[] construct, String methodName, Object[] arguments) {
		try {
			final var job = new ClassMethodInvoker(className, construct, methodName, arguments);
			job.start();
		} catch (Exception e) {
			e.printStackTrace();
		}
